...
//what the fuck is *numbers?
	int temp = 0;
	int i = 0;
	int j = 0;
	int *target = malloc(count * sizeof(int));
//why the fuck the asterisk?

	if (!target)
		die("Memory error.");
	
	memcpy(target, numbers, count * sizeof(int));
//what the fuck is memcpy?
	for (i = 0; i < count; i++) {
		for (j = 0; j < count - 1; j++) {
...
void test_sorting(int *numbers, int count, compare_cb cmp) {
	...
	printf("\n");
	free(sorted);
//The line below spits out raw hex for the bits the program usually returns//
/*	unsigned char *data = (unsigned char *)cmp;
	for (i = 0; i < 25; i++) {
		printf("%02x:", data[i]);
// what is that format specifier doing?
	}
	printf("\n"); */
}
